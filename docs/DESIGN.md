...
`/v1/logs` 接收遥测后立即返回符合 OTLP/HTTP 的 `200 {}`；故障监测产生的通知直接在进程内入队，不阻塞该请求等待 ntfy 网络发送。`/v1/notify` 在请求入队后等待该任务完成，并返回统一结构化结果。两个端点都只绑定回环地址，并分别校验请求格式与请求体大小。
...
4. 待确认故障超过 `failure_grace_seconds` 后发送一次优先级 `5` 通知；只有后台发送结果确认至少一个最终话题已送达后，才把该告警记为已处理。
5. 消息只包含模型、传输类型、HTTP 状态、尝试次数和经过截断/清洗的错误摘要，不包含用户提示词、模型输出或认证信息。
6. 相同会话内的相同故障指纹在 `dedupe_window_seconds` 内抑制重复通知；不同会话之间不去重、不聚合。
7. 后台发送异常或零送达时记录有界错误并保留待确认故障，在下一个宽限窗口重试，避免把“已入队”误当成“已送达”；部分送达记录警告并按已处理计，避免向已经成功的话题重复推送。
8. 只保留仍在宽限期内的故障和尚未过期的去重记录；恢复、告警处理完成或窗口过期后及时清理，不永久保存历史会话状态。
